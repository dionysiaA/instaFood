...
module.exports = task('build', ({ watch = false, onComplete } = {}) => new Promise((resolve) => {
  ...
  let watcher = chokidar.watch(['locales', 'src', 'package1.json', 'yarn.lock']);
  watcher.on('all', (event, src) => {
    // Reload the app if package1.json or yarn.lock files have changed (in watch mode)
    if (src === 'package1.json' || src === 'yarn.lock') {
