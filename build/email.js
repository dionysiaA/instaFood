...
const _ref = process.env.NODE_ENV === 'production' ? {
  ...
  streamTransport: true
      ...
      { from } = _ref,
      config = _objectWithoutProperties(_ref, ['from']);
...
_handlebars2.default.registerHelper('t', (key, options) => options.data.root.t(key, options.hash));
...
_fs2.default.readdirSync(baseDir).forEach(name => {
  if (_fs2.default.statSync(`${baseDir}/${name}`).isDirectory()) {
    templates.set(name, {
...
exports.default = {
  ...
  send(message, options) {
    return transporter.sendMail(_extends({}, message, options));
  }
};
